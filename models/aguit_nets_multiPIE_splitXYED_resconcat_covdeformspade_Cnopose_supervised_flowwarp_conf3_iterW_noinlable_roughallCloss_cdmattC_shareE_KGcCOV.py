...
#                de = lrelu(layer_norm(deconv(de, channels=self.channel * 4, kernel=4, stride=2, sn=False, scope='gen_conv_l21'), scope='gen_LN_l21'))
            ...
            de_0 = conv_2d(de, channels=self.channel * 2, kernel=3, stride=1, pad=1, sn=False, name='gen_out_0')
            de_0_shape = de_0.shape.as_list()
            de_0_up = tf.image.resize_images(de_0, (de_0_shape[1]*2, de_0_shape[2]*2))
#            de_0  = tanh(de_0)
            ...
            de_1 = conv_2d(tf.concat([de, de_0_up], -1), channels=self.channel, kernel=3, stride=1, pad=1,sn=False, name='gen_out_1')
            de_1 = conv_2d(de_1, channels=self.channel, kernel=3, stride=1, pad=1, sn=False, name='gen_out_11')
            de_1_shape = de_1.shape.as_list()
            de_1_up = tf.image.resize_images(de_1, (de_1_shape[1]*2, de_1_shape[2]*2))
#            de_1  = tanh(de_1)
            ...
            de_2 = conv_2d(tf.concat([de, de_1_up], -1), channels=self.channel, kernel=3, stride=1, pad=1, sn=False, name='gen_out_2')
            de_2 = conv_2d(de_2, channels=self.channel, kernel=3, stride=1, pad=1, sn=False, name='gen_out_21')
            de_2 = conv_2d(de_2, channels=3, kernel=1, stride=1, pad=0, sn=False, name='gen_out_22')
            de_2  = tanh(de_2)
...
#-----------------------------------------------decoder2: using corse image(target pose) feature to output crose image-------
            ...
            return de, [offset3_diff, offset2_diff, offset1_diff, offset1_diff_KG,\
                        offset3_conf_diff, offset2_conf_diff, offset1_conf_diff], [de_2]
    @property
    def vars(self):
        return tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name)
