...
from models.aguit_nets_multiPIE_splitXYED_resconcat_covdeformspade_Cnopose_supervised_flowwarp_conf3_iterW_noinlable_roughallCloss_cdmattC_shareE_KGcCOV import *
...
def coor_to_offset(coor, k=1):
    _, H, W, _ = coor.shape.as_list()
    out = tf.cast(tf.nn.top_k(coor, k=k).indices, tf.float32)
    offset_y = out/W
    offset_x = out%W
    offset = tf.concat([offset_y, offset_x], -1)
    
#        batch_size, height, width, channels = image.get_shape().as_list()
    grid_x, grid_y = array_ops.meshgrid(
        math_ops.range(W), math_ops.range(H))
    stacked_grid = math_ops.cast(
        array_ops.stack([grid_y, grid_x], axis=2), offset.dtype)
    batched_grid = array_ops.expand_dims(stacked_grid, axis=0)
    offset = batched_grid - offset
    return offset
...
class SNGAN():
    def __init__(self, encoder, generator, discriminator, latent_discriminator, batch_size, train_batch_size, log_dir='logs/imagenet',
        ...
        self.train_file = ['/datasets/multi_PIE_train_paired.tfrecords']
        self.imga, self.idlabela, self.labela, self.imgb, self.idlabelb, self.labelb = read_and_decode_multi_PIE_paired( self.train_file, self.size) 
        self.img_batcha, self.idlabel_batcha, self.label_batcha, self.img_batchb, self.idlabel_batchb, self.label_batchb = tf.train.shuffle_batch([self.imga, self.idlabela, self.labela, self.imgb, self.idlabelb, self.labelb, ],
                                                                  batch_size=self.batch_size,
                                                                  capacity=1000, min_after_dequeue=600)
        self.train_300wLP_file = ['/datasets/300w_LP_train_paired_shuf.tfrecords'] 
        self.imga_300wLP, self.idlabela_300wLP, self.labela_300wLP, self.imgb_300wLP, self.idlabelb_300wLP, self.labelb_300wLP = read_and_decode_300w_LP_paired( self.train_300wLP_file, self.size) 
        self.img_batcha_300wLP, self.idlabel_batcha_300wLP, self.label_batcha_300wLP, self.img_batchb_300wLP, self.idlabel_batchb_300wLP, self.label_batchb_300wLP = tf.train.shuffle_batch([self.imga_300wLP, self.idlabela_300wLP, self.labela_300wLP, self.imgb_300wLP, self.idlabelb_300wLP, self.labelb_300wLP],
                                                                  batch_size=self.batch_size,
                                                                  capacity=1000, min_after_dequeue=600) 
        self.test_file = ['/datasets/multi_PIE_test_paired.tfrecords'] 
        self.imga_test, self.idlabela_test, self.labela_test, self.imgb_test, self.idlabelb_test, self.labelb_test = read_and_decode_multi_PIE_paired(self.test_file, self.size)
        self.img_batcha_test, self.idlabel_batcha_test, self.label_batcha_test, self.img_batchb_test, self.idlabel_batchb_test, self.label_batchb_test = tf.train.batch(
                                                                    [self.imga_test,  self.idlabela_test, self.labela_test, self.imgb_test, self.idlabelb_test, self.labelb_test], batch_size=self.batch_size,
                                                                    capacity=1000)  
        
        self.test_file_show = ['/datasets/multi_PIE_test_show.tfrecords'] 
        self.img_show, self.label_show, self.idlabel_show = read_and_decode_multi_PIE(self.test_file_show, self.size) 
        self.img_batch_show, self.label_batch_show = tf.train.batch([self.img_show, self.label_show], batch_size=self.batch_size,
                                                                  capacity=1000)
        self.is_wganGP = True
        self.gp_lambda = 10
        ...
        self.Ymid = tf.placeholder(tf.int32, shape=[self.batch_size], name='Ymid')
        self.training = tf.placeholder(tf.bool,  name='training')
        self.Ya_matrix = tf.one_hot(self.Ya, self.y_dim)
        self.Yb_matrix = tf.one_hot(self.Yb, self.y_dim)
        self.Ymid_matrix = tf.one_hot(self.Ymid, self.y_dim)
        self.Y_diff_onehot = tf.one_hot(self.Yb - self.Ya + 8, self.y_diff_dim) 
        self.Y_mid_diff_onehot = tf.one_hot(self.Ymid - self.Ya + 8, self.y_diff_dim) 
        self.Y_diff_rec_onehot = tf.one_hot(self.Ya - self.Yb + 8, self.y_diff_dim)
        self.Y_diff_self_onehot = tf.one_hot(self.Ya - self.Ya + 8, self.y_diff_dim)
        ...
        self.x_b_fake, self.x_b_fake_diff_list, self.x_b_images = self.generator(self.z_f, labels=self.Yb_matrix, label_diff=self.Y_diff_onehot, statis = self.statis_a, batch_size=self.batch_size, train_batch_size=self.train_batch_size, pa=self.pa, pg=3, is_mlp=False, is_training=self.training) 
        ...
        self.x_aa_fake, self.x_aa_fake_diff_list, self.x_aa_images = self.generator(self.z_f, labels=self.Ya_matrix, label_diff=self.Y_diff_self_onehot, statis = self.statis_a, batch_size=self.batch_size, train_batch_size=self.train_batch_size, pa=self.pa, pg=3, is_mlp=False, is_training=self.training, reuse=True)
        ...
        self.x_a_fake , self.x_a_fake_diff_list, self.x_a_images = self.generator(self.z_f_rec, labels=self.Ya_matrix, label_diff=self.Y_diff_rec_onehot, statis = self.statis_b, batch_size=self.batch_size, train_batch_size=self.train_batch_size, pa=self.pa, pg=3, is_mlp=False, is_training=self.training, reuse=True)
...
#------flow warp image loss-----------------------------------------
        ...
        self.x_aa_fake_diff_offset1_flow = coor_to_offset(self.x_aa_fake_diff_offset1) 
        self.x_aa_fake_diff_offset1_KG_flow = tf.image.resize_images(self.x_aa_fake_diff_offset1_KG, (size_128, size_128))*4
        self.x_aa_fake_diff_offset1_flow = tf.image.resize_images(self.x_aa_fake_diff_offset1_flow, (size_128, size_128))*4
        self.x_aa_fake_diff_offset2_flow = tf.image.resize_images(self.x_aa_fake_diff_offset2, (size_128, size_128))*2
        self.x_aa_fake_diff_offset3_flow = self.x_aa_fake_diff_offset3
        
        self.x_b_fake_diff_offset1_flow = coor_to_offset(self.x_b_fake_diff_offset1) 
        self.x_b_fake_diff_offset1_KG_flow = tf.image.resize_images(self.x_b_fake_diff_offset1_KG, (size_128, size_128))*4
        self.x_b_fake_diff_offset1_flow = tf.image.resize_images(self.x_b_fake_diff_offset1_flow, (size_128, size_128))*4
        self.x_b_fake_diff_offset2_flow = tf.image.resize_images(self.x_b_fake_diff_offset2, (size_128, size_128))*2
        self.x_b_fake_diff_offset3_flow = self.x_b_fake_diff_offset3*1
        
        self.x_b_fake_diff_offset1_KG_flow = self.x_b_fake_diff_offset1_KG_flow - self.x_aa_fake_diff_offset1_KG_flow
        self.x_b_fake_diff_offset1_flow = self.x_b_fake_diff_offset1_flow - self.x_aa_fake_diff_offset1_flow
        self.x_b_fake_diff_offset2_flow = self.x_b_fake_diff_offset2_flow - self.x_aa_fake_diff_offset2_flow
        self.x_b_fake_diff_offset3_flow = self.x_b_fake_diff_offset3_flow - self.x_aa_fake_diff_offset3_flow
        self.x_b_fake_diff_offset_all_flow = self.x_b_fake_diff_offset1_KG_flow + self.x_b_fake_diff_offset1_flow +\
                                             self.x_b_fake_diff_offset2_flow + self.x_b_fake_diff_offset3_flow
        ...
        self.all_diff_fake_b_128_show = all_diff_fake_b_128
        self.all_diffor_fake_b_128 = self.x_b_fake_diff_conf3*all_diff_fake_b_128 +\
        (1-self.x_b_fake_diff_conf3)*tf.image.resize_images(self.x_b_fake, (size_128, size_128))
        ...
        self.x_b_rough2= self.x_b_images[0]
        self.x_aa_rough2 = self.x_aa_images[0]
        self.x_a_rough2 = self.x_a_images[0]
        
        self.dis_f_r2, self.dis_f_cls_r2 = self.discriminator(self.x_b_rough2, reuse = True)
        self.loss_rec_rough0 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(\
                                logits=self.dis_f_cls_r2, \
                                labels=self.Yb_matrix))
        self.loss_rec_rough = self.loss_rec_rough0
        self.log_vars.append(("loss_rec_rough", self.loss_rec_rough)) 
...
#        self.loss_rec = tf.reduce_mean(tf.square(self.Xb - self.x_b_fake)) 
        ...
        self.loss_rec_rough = 10*tf.reduce_mean(tf.square(self.Xb - self.x_b_rough2)) + \
        tf.reduce_mean(tf.square(self.Xa - self.x_a_rough2)) + tf.reduce_mean(tf.square(self.Xa - self.x_aa_rough2))
        self.log_vars.append(("loss_rec_pixel_rough", self.loss_rec_rough))         
        #self.x_b_fake_flow = flow_warping(self.Xa, self.x_b_fake_offset3)
...
#        self.loss_dis_ac = tf.reduce_mean(tf.square(self.dis_real_cls-self.Yb_matrix))+tf.reduce_mean(tf.square(self.dis_f_cls-self.Yb_matrix))
        ...
        with tf.control_dependencies(update_ops): 
            ...
            self.gen_solver = self.gen_opt.minimize(self.loss_gen + self.loss_rec*5 + self.loss_rec_rough*5 +\
                                                    self.loss_rec_rough*5 +\
                                                    5*self.loss_rec_f + \
            ...
            self.enc_solver = self.gen_opt.minimize(self.loss_gen + self.loss_VI + self.loss_rec*5 \
                                                    + self.loss_rec_rough*5 + \
                                                    5*self.loss_rec_f + self.loss_rec_rough*5 +\
    ...
    def test(self, sample_folder, model_path, training_iters=50000, batch_size=64, n_dis = 1, n_samples = 16, restore=False):
        ...
        for i in range(1):
            ...
            for iter in tqdm(range(training_iters)): 
                ...
                Y_batch_or = (2<=Y_batch_a) * ( Y_batch_a<=10) * (2<=Y_batch_b) * ( Y_batch_b<=10)
                X_batch_a_OK = X_batch_a[Y_batch_or]
                X_batch_b_OK = X_batch_b[Y_batch_or]
                Y_batch_a_OK = Y_batch_a[Y_batch_or]
                Y_batch_b_OK = Y_batch_b[Y_batch_or]
                while(X_batch_a_OK.shape[0]<batch_size): 
                    X_batch_a, Y_batch_a, X_batch_b, Y_batch_b = self.sess.run([self.img_batcha_test, self.label_batcha_test, self.img_batchb_test, self.label_batchb_test])
                    Y_batch_or = (2<=Y_batch_a) * ( Y_batch_a<=10) * (2<=Y_batch_b) * ( Y_batch_b<=10)
                    X_batch_a_OK1 = X_batch_a[Y_batch_or]
                    X_batch_b_OK1 = X_batch_b[Y_batch_or]
                    Y_batch_a_OK1 = Y_batch_a[Y_batch_or]
                    Y_batch_b_OK1 = Y_batch_b[Y_batch_or]
                    X_batch_a_OK = np.concatenate([X_batch_a_OK, X_batch_a_OK1], 0)
                    X_batch_b_OK = np.concatenate([X_batch_b_OK, X_batch_b_OK1], 0)
                    Y_batch_a_OK = np.concatenate([Y_batch_a_OK, Y_batch_a_OK1], 0)
                    Y_batch_b_OK = np.concatenate([Y_batch_b_OK, Y_batch_b_OK1], 0)
                X_batch_a = X_batch_a_OK[:batch_size]
                X_batch_b = X_batch_b_OK[:batch_size]
                Y_batch_a = Y_batch_a_OK[:batch_size]-2
                Y_batch_b = Y_batch_b_OK[:batch_size]-2
    ...
    def train(self, sample_folder, training_iters=50000, batch_size=64, n_dis = 1, n_samples = 16, restore=False):
        ...
        for iter in range(restore_iter, training_iters):
            ...
            if multipie:
                ...
                X_batch_a, Y_batch_a, X_batch_b, Y_batch_b = self.sess.run([self.img_batcha_300wLP, self.label_batcha_300wLP, self.img_batchb_300wLP, self.label_batchb_300wLP])
                Y_batch_a = 8 - Y_batch_a
                Y_batch_b = 8 - Y_batch_b
            X_batch_a = X_batch_a / 255.0 * 2 - 1
            X_batch_b = X_batch_b / 255.0 * 2 - 1  
            ...
            if iter % 100 == 0 or iter < 100:
                fetch_list = [self.loss_VI, self.loss_rec, self.loss_rec_f, self.loss_rec_f2, self.loss_gen, self.loss_dis, self.loss_dis_ac, self.loss_rec_rough, self.loss_rec_warp]
                loss_VI_curr, loss_rec_curr, loss_rec_f_curr, loss_rec_f2_curr,\
                loss_gen_curr, loss_dis_curr, loss_dis_ac_curr, loss_rec_rough_curr, loss_rec_warp_curr = \
                ...
                print('TRAIN Iter: {};  loss_vi: {:.4}; loss_rec: {:.4}; loss_rec_f: {:.4}; loss_rec_f2: {:.4}; loss_gen: {:.4}; \
                      loss_dis: {:.4};  loss_dis_ac: {:.4};  loss_rec_rough: {:.4}; loss_rec_warp: {:.4}'.
                ...
                if iter % 500 == 0:
                    ...
                    X_batch_a = X_batch_a / 255.0 * 2 - 1
                    X_batch_b = X_batch_b / 255.0 * 2 - 1  
                    fetch_list = [self.loss_VI, self.loss_rec, self.loss_rec_f, self.loss_rec_f2, self.loss_gen, self.loss_dis, self.loss_dis_ac, self.loss_rec_warp, self.loss_rec_rough]
                    loss_VI_curr, loss_rec_curr, loss_rec_f_curr, loss_rec_f2_curr, loss_gen_curr, loss_dis_curr, loss_dis_ac_curr, loss_rec_warp_curr, loss_rec_rough_curr = self.sess.run(fetch_list,
                    ...
                    print('TESt Iter: {}; loss_vi: {:.4}; loss_rec: {:.4}; loss_rec_f: {:.4}; loss_rec_f2: {:.4}; loss_gen: {:.4};\
                          loss_dis: {:.4}; loss_dis_ac: {:.4}; loss_rec_warp: {:.4}; loss_rec_rough: {:.4}'.
                      format(iter, loss_VI_curr, loss_rec_curr, loss_rec_f_curr, loss_rec_f2_curr, loss_gen_curr, loss_dis_curr, \
                             loss_dis_ac_curr, loss_rec_warp_curr, loss_rec_rough_curr))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--y_dim', type=int, default=9, help='label dimension')
    ...
    if args.mode=='training':
        ...
        gan.test(sample_folder+'/test', model_path=args.model_path, batch_size=batch_size, training_iters=3133, restore = False)
